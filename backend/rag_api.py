...
def chat_normal(query):
    ...
    response = ollama.chat(model="llama3.1:8b-instruct-fp16", messages=[{"role": "user", "content": query}])
...
def chat_with_rag(query, retrieved_slides=None):
    ...
    response = ollama.chat(model="llama3.1:8b-instruct-fp16", messages=[{"role": "user", "content": prompt}])
